...
def login():
    try:
        ...
        if user and user.password == password:
            # Generate JWT token - Fix the datetime warning and token encoding
            token_payload = {
            ...
            # Make sure your app has a SECRET_KEY defined and encode as string
            token = jwt.encode(
                token_payload, 
...
def add_course():
    try:
        ...
        # التحقق من وجود حقول اليوم والوقت (غير إلزامية ولكن يجب التحقق منها)
        if 'day' not in data:
            data['day'] = ''
